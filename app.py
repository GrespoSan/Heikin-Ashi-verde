...
def fetch_data(symbol, interval):
    try:
        ...
        # Filtro per rimuovere candele sporche (domeniche o dati vuoti)
...
def analyze_stock(symbol):
    ...
    rec_is_verde = c_rec['Close'] > c_rec['Open']
    prev_is_rossa = c_prev['Close'] < c_prev['Open']

    # Mostriamo i segnali solo se c'è inversione (Verde dopo Rossa)
    if rec_is_verde and prev_is_rossa:
        return {
            "Symbol": symbol,
            "Trend": "🟢", # Pallino generale segnale
...
if results:
    ...
    # Riordino colonne per chiarezza
    order = ["Symbol", "Trend", "Data Precedente", "Colore Prec", "Data Recente", "Colore Rec", "HA_Open_Rec", "HA_Close_Rec"]
    st.table(df_res[order])
    ...
    st.info("Nota: Di domenica usa 'Live' per analizzare il Venerdì.")